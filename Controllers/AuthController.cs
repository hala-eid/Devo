    ...
    public AuthController(DevoDbContext context, JwtService jwtService)
    {
      ...
      _jwtService = jwtService; // <-- assign it here
        ...
        public async Task<IActionResult> Register([FromBody] User request)
        {
            ...
            // 4️⃣ Hash the password once
            var hashedPassword = HashPassword(request.PasswordHash);

            // 5️⃣ Create the database entity object
            var newUser = new User // Renamed to 'newUser' to avoid confusion with the 'user' variable in the response
            {
            ...
            // 6️⃣ Save to database once
            _context.Users.Add(newUser);
            await _context.SaveChangesAsync();

            // 7️⃣ Generate token
          
            // 8️⃣ Return the response that your Angular AuthService expects
            return Ok(new
            {
                success = true,
